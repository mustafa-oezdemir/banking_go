...
export function TransferWizard({ accounts, onComplete }: { accounts: Account[]; onComplete: () => Promise<void> }) {
	...
	const next = async () => {
		...
		if (step === 2) {
			const nameValidation = plainTextError(form.beneficiaryName, "Empfängername", { required: true, min: 1, max: 140 });
			if (nameValidation) return setError(nameValidation);
			if (normalizeIBAN(form.beneficiaryIBAN).length < 15) return setError("Eine gültige IBAN ist erforderlich.");
		...
		if (step === 3) {
			...
			const purposeValidation = plainTextError(form.purpose, "Verwendungszweck", { max: 140 });
			if (purposeValidation) return setError(purposeValidation);
			const referenceValidation = plainTextError(form.creditorReference, "Gläubigerreferenz", { max: 35 });
			if (referenceValidation) return setError(referenceValidation);
	...
	const submit = async () => {
		...
		try {
			const common = {
				...
				beneficiary_name: normalizePlainText(form.beneficiaryName),
				...
				purpose: normalizePlainText(form.purpose) || undefined,
				creditor_reference: normalizePlainText(form.creditorReference) || undefined,

...
func (store *Store) ExecTx(ctx context.Context, fn func(q *sqlc.Queries) error) error {
	return store.ExecTxWithHandle(ctx, func(q *sqlc.Queries, _ sqlc.DBTX) error {
		return fn(q)
	})
}

// ExecTxWithHandle runs fn in a serializable transaction and also exposes the
// transaction handle for security-sensitive statements that are not generated by sqlc.
func (store *Store) ExecTxWithHandle(ctx context.Context, fn func(q *sqlc.Queries, executor sqlc.DBTX) error) error {
...
func (store *Store) execTxOnce(ctx context.Context, fn func(q *sqlc.Queries, executor sqlc.DBTX) error) error {

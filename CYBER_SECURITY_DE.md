...
| Datenbank-Service-Ownership | `docker-compose.yml`, `docker/postgres/provision-service-roles.sh`, Migration `000015` | `banking_app`, `identity_app` und `notification_app` besitzen DML-Rechte nur in ihrem eigenen Schema. Anwendungscontainer verwenden nicht den PostgreSQL-Administrator; schemaübergreifende Abfragen/Schreibvorgänge werden durch die Datenbank abgewiesen. |
...
| Feste manuelle Abfragen      | `backend/internal/platform/database/admin.go`, `backend/internal/platform/identitystore/store.go` | Auch manuelle SQL-Anweisungen verwenden Platzhalterparameter.                                                                               |
...
| Service-Grenze und Replay | `backend/internal/platform/identityapi/handler.go`, `backend/internal/platform/httpapi/security.go`, `backend/cmd/auth_boundary_test.go` | Credential-Endpunkte und JWT-Ausstellung liegen ausschließlich bei Identity. Logout und Passwortänderung widerrufen die Session-Generation; ein Replay des alten JWT wird abgelehnt. Banking registriert keine alten Auth-Routen. |
| Auth-Response und CSRF | `backend/internal/platform/identityapi/handler.go` | Auth-Antworten verwenden `no-store`; Cookie-authentifizierter Logout und Passwortänderungen erfordern den CSRF-Header und Fetch-Metadata-Prüfung. |
...
| Kontrolle                        | Datei                                                           | Beschreibung                                                                                                                |
| -------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| bcrypt-Hash                      | `backend/internal/platform/identityapi/handler.go`, `credentials.go`         | Passwörter werden nicht im Klartext gespeichert, sondern mit bcrypt gehasht.                                               |

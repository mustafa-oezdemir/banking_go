...
| HTML-Escaping und lokales Abfangen von E-Mails | `backend/internal/platform/email/resend.go`, `backend/internal/platform/email/smtp.go`, `docker-compose.dev.yml` | Name, Konto, Betrag, Gegenpartei und Verwendungszweck werden mit`html.EscapeString` kodiert. Lokal werden Nachrichten über MailHog-SMTP abgefangen und nicht ins Internet versendet; in Produktion wird Resend HTTPS verwendet. |
